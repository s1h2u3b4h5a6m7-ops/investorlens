...
function openMap(){
  showPage('map-page', 'fwd');
  renderMap();
}

/* Session 6: one chain at a time, with an index beside it — the same shell the
   force lens uses, because it answers the same shape of question: here is a set,
   here is the one you picked, read at full size. All four chains rendered at
   once meant the third and fourth were never seen, and every title sat at card
   size while being the subject of its own screen. */
var currentChain = null;

function renderMap(){
  var ch = CHAINMAP.filter(function(x){ return x.id === currentChain; })[0] || CHAINMAP[0];
  currentChain = ch.id;

  var ix = document.getElementById('map-index');
  if(ix){
    ix.innerHTML = CHAINMAP.map(function(x){
      var n = (x.kind === 'ownership')
        ? (x.pairs || []).length * 2
        : (x.stages || []).reduce(function(a, s){ return a + s.nodes.length; }, 0);
      return '<button class="frc-ix-item' + (x.id === ch.id ? ' active' : '') + '" data-chain="' + esc(x.id) + '">'
        + '<span>' + esc(x.title) + '</span><em>' + n + '</em></button>';
    }).join('');
    ix.querySelectorAll('.frc-ix-item').forEach(function(b){
      b.addEventListener('click', function(){ currentChain = b.getAttribute('data-chain'); renderMap(); });
    });
  }
  ...
  var idx = 0;
  for(var q = 0; q < CHAINMAP.length; q++){ if(CHAINMAP[q].id === ch.id){ idx = q + 1; break; } }

  var el = document.getElementById('map-content');
  el.innerHTML = '<div class="frc-eyebrow">'
      + '<span class="frc-ix-of">Chain ' + idx + ' / ' + CHAINMAP.length + '</span>'
