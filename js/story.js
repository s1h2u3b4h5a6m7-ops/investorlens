...
var STORY = (function(){
  ...
  var TABS = [
    ...
    {id:'st-sectors',   label:'Sectors',     icon:'t1', moves:'panel-sectors',
     title:'Browse by sector', blurb:'Groups of businesses that face the same customers and the same costs. Ordered by how many of them we cover, largest first \u2014 the rail is that share of every company on the platform, not a share of the market and not a ranking of the businesses.'},
  ...
  function fillChanged(){
    ...
    /* Session 8: the page becomes two tabs. Tab 1 is exactly what this page
       already was — the ledger, then Live factors, same two builders, same
       order, same containers. Tab 2 is the pooled headline river, which is
       machine-collected and sits behind its own fence.
       The tabs are a VIEW SWITCH, not a re-render: switching only toggles
       [hidden], so tab 1 is never rebuilt and its click handlers survive. */
    box.innerHTML = '<div class="st-t2bar" role="tablist">'
      +   '<button class="st-t2 is-on" type="button" role="tab" aria-selected="true" data-pane="currency">Data currency</button>'
  ...
  /* THE FOURTH TIME THIS LESSON APPLIES. map-page, st-changed and forces-page
     each opened empty because a lazily-built surface had its builder invoked on
     the ROUTE instead of at the DESTINATION. A tab is a destination too: the
     river is built on the first switch to it, here, and not before. */
  function wireFreshTabs(box){
    var btns = box.querySelectorAll('.st-t2');
    Array.prototype.forEach.call(btns, function(b){
      b.addEventListener('click', function(){
        var want = b.getAttribute('data-pane');
        Array.prototype.forEach.call(btns, function(x){
          var on = (x === b);
          if(on){ x.classList.add('is-on'); } else { x.classList.remove('is-on'); }
          x.setAttribute('aria-selected', on ? 'true' : 'false');
        });
        showPane('st-pane-currency', want === 'currency');
        showPane('st-pane-news',     want === 'news');
        if(want === 'news') fillNewsPane();
      });
    });
  }
  ...
  /* Its own guard, not the page's. If the robot has written nothing yet the
     river renders an honest empty state and returns 0, leaving data-filled
     unset so the next visit tries again rather than caching the emptiness. */
  function fillNewsPane(){
    var el = document.getElementById('st-news-box');
    if(!el || el.getAttribute('data-filled')) return 0;
    if(typeof buildNewsFeed !== 'function') return 0;
    var n = 0;
    try { n = buildNewsFeed(el); }
    catch(e){ if(window.console && console.warn) console.warn('news feed failed:', e); }
    if(n) el.setAttribute('data-filled', '1');
    return n;
  }
